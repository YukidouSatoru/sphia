...
static void my_application_activate(GApplication* application) {
  ...
  if (existing_window) {
    ...
    if (use_header_bar) {
      ...
      gtk_header_bar_set_title(header_bar, "Sphia");
    ...
    gtk_widget_set_size_request(GTK_WIDGET(window), 980, 720);
    gtk_widget_show(GTK_WIDGET(window));

    if (g_file_test("assets", G_FILE_TEST_IS_DIR)) {
      gtk_window_set_icon_from_file(window, "assets/app_icon.png", NULL);
    } else {
      gtk_window_set_icon_from_file(window, "data/flutter_assets/assets/app_icon.png", NULL);
    }
